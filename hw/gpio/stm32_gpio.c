...
static uint8_t stm32_gpio_get_pin_config(Stm32Gpio *s, unsigned pin) {
   ...
   * one 64 bit value.
   */
  uint64_t cr_64 = ((uint64_t)s->GPIOx_CRy[1] << 32) | s->GPIOx_CRy[0];
...
static const MemoryRegionOps stm32_gpio_ops = {.read = stm32_gpio_read,
...
static int stm32_gpio_init(SysBusDevice *dev) {
  ...
  memory_region_init_io(&s->iomem, OBJECT(s), &stm32_gpio_ops, s, "gpio",
                        0x03ff);
