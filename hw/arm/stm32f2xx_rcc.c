...
#define DPRINTF(fmt, ...)                                                      \
  do {                                                                         \
    printf("STM32F2XX_RCC: " fmt, ##__VA_ARGS__);                              \
  } while (0)
#else
#define DPRINTF(fmt, ...)
#endif

#define IS_RESET_VALUE(new_value, mask, reset_value)                           \
  ((new_value & mask) == (mask & reset_value))
...
static void stm32_rcc_periph_enable(Stm32f2xxRcc *s, uint32_t new_value,
                                    bool init, int periph, uint32_t bit_mask) {
  // printf("rcc set 0x%x %s %d %x: %sable\n", new_value,
  // s->PERIPHCLK[periph]->name, periph, bit_mask, IS_BIT_SET(new_value,
  // bit_mask)?"en":"dis");
 ...
 * states.
 */
static void stm32_rcc_RCC_CR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                   bool init) {
  ...
  if ((clktree_is_enabled(s->PLLCLK) && !new_PLLON) &&
    ...
    stm32_hw_warn(
        "PLL cannot be disabled while it is selected as the system clock.");
  }
  ...
  if ((clktree_is_enabled(s->HSECLK) && !new_HSEON) &&
      (s->RCC_CFGR_SW == SW_HSE_SELECTED ||
       s->RCC_CFGR_SW == SW_PLL_SELECTED)) {
    stm32_hw_warn("HSE oscillator cannot be disabled while it is driving the "
                  "system clock.");
  }
  ...
  if ((clktree_is_enabled(s->HSECLK) && !new_HSEON) &&
      (s->RCC_CFGR_SW == SW_HSI_SELECTED ||
       s->RCC_CFGR_SW == SW_PLL_SELECTED)) {
    stm32_hw_warn("HSI oscillator cannot be disabled while it is driving the "
                  "system clock.");
  }
...
static void stm32_rcc_RCC_PLLCFGR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                        bool init) {
  /* PLLM division factor */
  const uint8_t new_PLLM =
      (new_value & RCC_PLLCFGR_PLLM_MASK) >> RCC_PLLCFGR_PLLM_START;
  if (new_PLLM <= 1) {
    hw_error("PLLM division factor cannot be 0 or 1. Given: %u", new_PLLM);
  }

  /* PLLN multiplication factor */
  const uint16_t new_PLLN =
      (new_value & RCC_PLLCFGR_PLLN_MASK) >> RCC_PLLCFGR_PLLN_START;
  if (new_PLLN <= 1 || new_PLLN >= 433) {
    hw_error("PLLN multiplication factor must be between 2 and 432 "
             "(inclusive). Given: %u",
             new_PLLN);
  }
  ...
  const uint8_t new_PLLP =
      2 + (2 * ((new_value & RCC_PLLCFGR_PLLP_MASK) >> RCC_PLLCFGR_PLLP_START));

  /* Warn in case of illegal writes: */
  if (init == false) {
    const bool are_disabled = (!clktree_is_enabled(
        s->PLLCLK) /* && TODO: !clktree_is_enabled(s->PLLI2SCLK) */);
    if (are_disabled == false) {
      const char *warning_fmt =
          "Can only change %s while PLL and PLLI2S are disabled";
...
static void stm32_rcc_RCC_PLLI2SCFGR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                           bool init) {
  /* PLLR division factor */
  const uint16_t new_PLLR =
      (new_value & RCC_PLLI2SCFGR_PLLR_MASK) >> RCC_PLLI2SCFGR_PLLR_START;
  if (new_PLLR < 2 || new_PLLR > 7) {
    hw_error("PLLR multiplication factor must be between 2 and 7. Given: %u",
             new_PLLR);
  }

  /* PLLQ division factor */
  const uint16_t new_PLLQ =
      (new_value & RCC_PLLI2SCFGR_PLLQ_MASK) >> RCC_PLLI2SCFGR_PLLQ_START;
  if (new_PLLQ > 15) {
  ...
  /* PLLN multiplication factor */
  const uint16_t new_PLLN =
      (new_value & RCC_PLLI2SCFGR_PLLN_MASK) >> RCC_PLLI2SCFGR_PLLN_START;
  ...
  if (init == false) {
    const bool are_disabled = (!clktree_is_enabled(
        s->PLLI2SCLK) /* && TODO: !clktree_is_enabled(s->PLLI2SCLK) */);
    if (are_disabled == false) {
      const char *warning_fmt =
          "Can only change %s while PLL and PLLI2S are disabled";
  ...
  WARN_UNIMPLEMENTED(new_value, RCC_PLLI2SCFGR_PLLQ_MASK,
                     RCC_PLLI2SCFGR_RESET_VALUE);
}

static uint32_t stm32_rcc_RCC_CFGR_read(Stm32f2xxRcc *s) {
  return (s->RCC_CFGR_PPRE2 << RCC_CFGR_PPRE2_START) |
...
static void stm32_rcc_RCC_CFGR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                     bool init) {
  /* PPRE2 */
...
static uint32_t stm32_rcc_RCC_CIR_read(Stm32f2xxRcc *s) { return s->RCC_CIR; }

static void stm32_rcc_RCC_CIR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                    bool init) {
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_CIR_CSSC_BIT, RCC_CIR_RESET_VALUE);
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_CIR_PLLI2SRDYC_BIT,
                     RCC_CIR_RESET_VALUE);
  ...
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_CIR_PLLI2SRDYIE_BIT,
                     RCC_CIR_RESET_VALUE);
  ...
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_CIR_PLLI2SRDYF_BIT,
                     RCC_CIR_RESET_VALUE);
...
static void stm32_rcc_RCC_AHB1ENR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                        bool init) {
  clktree_set_enabled(s->PERIPHCLK[STM32_DMA2],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_DMA2EN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_DMA1],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_DMA1EN_BIT));

  clktree_set_enabled(s->PERIPHCLK[STM32_CRC],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_CRCEN_BIT));

  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOK],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOKEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOJ],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOJEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOI],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOIEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOH],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOHEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOG],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOGEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOF],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOFEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOE],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOEEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOD],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIODEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOC],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOCEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOB],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOBEN_BIT));
  clktree_set_enabled(s->PERIPHCLK[STM32_GPIOA],
                      IS_BIT_SET(new_value, RCC_AHB1ENR_GPIOAEN_BIT));

  s->RCC_AHB1ENR = new_value;

  WARN_UNIMPLEMENTED(new_value, 1 << RCC_AHB1ENR_OTGHSULPIEN_BIT,
                     RCC_AHB1ENR_RESET_VALUE);
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_AHB1ENR_OTGHSEN_BIT,
                     RCC_AHB1ENR_RESET_VALUE);
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_AHB1ENR_ETHMACPTPEN_BIT,
                     RCC_AHB1ENR_RESET_VALUE);
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_AHB1ENR_ETHMACRXEN_BIT,
                     RCC_AHB1ENR_RESET_VALUE);
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_AHB1ENR_ETHMACTXEN_BIT,
                     RCC_AHB1ENR_RESET_VALUE);
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_AHB1ENR_ETHMACEN_BIT,
                     RCC_AHB1ENR_RESET_VALUE);
  WARN_UNIMPLEMENTED(new_value, 1 << RCC_AHB1ENR_BKPSRAMEN_BIT,
                     RCC_AHB1ENR_RESET_VALUE);
}

static void stm32_rcc_RCC_AHB2ENR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                        bool init) {
...
static void stm32_rcc_RCC_AHB3ENR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                        bool init) {
...
static void stm32_rcc_RCC_BDCR_writeb0(Stm32f2xxRcc *s, uint8_t new_value,
                                       bool init) {
...
static void stm32_rcc_RCC_BDCR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                     bool init) {
...
static void stm32_rcc_RCC_CSR_write(Stm32f2xxRcc *s, uint32_t new_value,
                                    bool init) {
...
static const MemoryRegionOps stm32_rcc_ops = {.read = stm32_rcc_read,
...
static void stm32_rcc_init_clk(Stm32f2xxRcc *s) {
  ...
  s->IWDGCLK = clktree_create_clk("IWDGCLK", 1, 1, false, CLKTREE_NO_MAX_FREQ,
                                  0, s->LSICLK, NULL);
  s->RTCCLK = clktree_create_clk("RTCCLK", 1, 1, false, CLKTREE_NO_MAX_FREQ,
                                 CLKTREE_NO_INPUT, s->LSECLK, s->LSICLK,
                                 s->HSECLK, NULL);

  s->PLLM = clktree_create_clk("PLLM", 1, 16, true, CLKTREE_NO_MAX_FREQ, 0,
                               s->HSICLK, s->HSECLK, NULL);
  s->PLLCLK =
      clktree_create_clk("PLLCLK", 1, 2, false, 120000000, 0, s->PLLM, NULL);
  s->PLL48CLK =
      clktree_create_clk("PLL48CLK", 1, 1, false, 48000000, 0, s->PLLM, NULL);

  s->PLLI2SM = clktree_create_clk("PLLI2SM", 1, 16, true, CLKTREE_NO_MAX_FREQ,
                                  0, s->PLLM, NULL);
  s->PLLI2SCLK = clktree_create_clk("PLLI2SCLK", 1, 2, false, 120000000, 0,
                                    s->PLLI2SM, NULL);

  s->SYSCLK =
      clktree_create_clk("SYSCLK", 1, 1, true, 168000000, CLKTREE_NO_INPUT,
                         s->HSICLK, s->HSECLK, s->PLLCLK, NULL);

  // HCLK: to AHB bus, core memory and DMA
  s->HCLK =
      clktree_create_clk("HCLK", 0, 1, true, 168000000, 0, s->SYSCLK, NULL);
  clktree_adduser(s->HCLK, hclk_upd_irq[0]);

  // Clock source for APB1 peripherals:
  s->PCLK1 =
      clktree_create_clk("PCLK1", 0, 1, true, 30000000, 0, s->HCLK, NULL);

  // Clock source for APB2 peripherals:
  s->PCLK2 =
      clktree_create_clk("PCLK2", 0, 1, true, 60000000, 0, s->HCLK, NULL);

  /* Peripheral clocks */
  s->PERIPHCLK[STM32_GPIOA] = clktree_create_clk(
      "GPIOA", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOB] = clktree_create_clk(
      "GPIOB", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOC] = clktree_create_clk(
      "GPIOC", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOD] = clktree_create_clk(
      "GPIOD", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOE] = clktree_create_clk(
      "GPIOE", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOF] = clktree_create_clk(
      "GPIOF", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOG] = clktree_create_clk(
      "GPIOG", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOH] = clktree_create_clk(
      "GPIOH", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOI] = clktree_create_clk(
      "GPIOI", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOJ] = clktree_create_clk(
      "GPIOJ", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_GPIOK] = clktree_create_clk(
      "GPIOK", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);

  s->PERIPHCLK[STM32_CRC] = clktree_create_clk(
      "CRC", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);

  s->PERIPHCLK[STM32_DMA1] = clktree_create_clk(
      "DMA1", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_DMA2] = clktree_create_clk(
      "DMA2", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);

  s->PERIPHCLK[STM32_SYSCFG] = clktree_create_clk(
      "SYSCFG", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK2, NULL);

  s->PERIPHCLK[STM32_UART1] = clktree_create_clk(
      "UART1", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK2, NULL);
  s->PERIPHCLK[STM32_UART2] = clktree_create_clk(
      "UART2", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK1, NULL);
  s->PERIPHCLK[STM32_UART3] = clktree_create_clk(
      "UART3", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK1, NULL);
  s->PERIPHCLK[STM32_UART4] = clktree_create_clk(
      "UART4", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK1, NULL);
  s->PERIPHCLK[STM32_UART5] = clktree_create_clk(
      "UART5", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK1, NULL);
  s->PERIPHCLK[STM32_UART6] = clktree_create_clk(
      "UART6", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK2, NULL);
  s->PERIPHCLK[STM32_UART7] = clktree_create_clk(
      "UART7", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK1, NULL);
  s->PERIPHCLK[STM32_UART8] = clktree_create_clk(
      "UART8", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK1, NULL);

  s->PERIPHCLK[STM32_DCMI_PERIPH] = clktree_create_clk(
      "DCMI", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_CRYP_PERIPH] = clktree_create_clk(
      "CRYP", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_HASH_PERIPH] = clktree_create_clk(
      "HASH", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
  s->PERIPHCLK[STM32_RNG_PERIPH] = clktree_create_clk(
      "RNG", 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);

  s->PERIPHCLK[STM32_FSMC] = clktree_create_clk(
      "FSMC", 1, 2, false, CLKTREE_NO_MAX_FREQ, 0, s->HCLK, NULL);
}

static int stm32_rcc_init(SysBusDevice *dev) {
  Stm32f2xxRcc *s = FROM_SYSBUS(Stm32f2xxRcc, dev);

  memory_region_init_io(&s->iomem, OBJECT(s), &stm32_rcc_ops, s, "rcc",
                        0x40023BFF - 0x40023800 + 1);
...
static TypeInfo stm32_rcc_info = {.name = "stm32f2xx_rcc",
