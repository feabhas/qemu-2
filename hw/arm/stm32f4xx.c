...
static const char *stm32f4xx_periph_name_arr[] = {
    ...
    ENUM_STRING(STM32_UART7),       ENUM_STRING(STM32_UART8),
...
void stm32f4xx_init(ram_addr_t flash_size, /* in KBytes */
                    ram_addr_t ram_size,   /* in KBytes */
                    const char *kernel_filename, Stm32Gpio **stm32_gpio,
                    const uint32_t *gpio_idr_masks, Stm32Uart **stm32_uart,
                    Stm32Timer **stm32_timer, DeviceState **stm32_rtc,
                    uint32_t osc_freq, uint32_t osc32_freq,
                    struct stm32f4xx *stm, ARMCPU **cpu) {
  ...
  if (dinfo) {
    f2xx_flash_register(blk_by_legacy_dinfo(dinfo), STM32_FLASH_ADDR_START,
                        flash_size * 1024);
  }
  ...
  memory_region_init_alias(flash_alias,             /* alias memory region */
                           NULL,                    /* owner */
  ...
  stm32_init_periph(rcc_dev, STM32_RCC_PERIPH, /* periph index, not used */
                    0x40023800,                /* base address */
                    qdev_get_gpio_in(nvic, STM32_RCC_IRQ)); /* irq */

  /* Setup GPIOs */
  DeviceState **gpio_dev =
      (DeviceState **)g_malloc0(sizeof(DeviceState *) * STM32F4XX_GPIO_COUNT);
  for (i = 0; i < STM32F4XX_GPIO_COUNT; i++) {
    ...
    stm32_init_periph(gpio_dev[i], periph, 0x40020000 + (i * 0x400),
                      NULL /*irq*/);
  ...
  //    rcc_dev, gpio_dev, afio_dev, 0x40011000, pic[STM32_UART1_IRQ]);
  //    stm32_uart[STM32_UART2_INDEX] = stm32_create_uart_dev(STM32_UART2,
  //    rcc_dev, gpio_dev, afio_dev, 0x40004400, pic[STM32_UART2_IRQ]);
  //    stm32_uart[STM32_UART3_INDEX] = stm32_create_uart_dev(STM32_UART3,
  //    rcc_dev, gpio_dev, afio_dev, 0x40004800, pic[STM32_UART3_IRQ]);
  //    stm32_uart[STM32_UART4_INDEX] = stm32_create_uart_dev(STM32_UART4,
  //    rcc_dev, gpio_dev, afio_dev, 0x40004C00, pic[STM32_UART4_IRQ]);
  //    stm32_uart[STM32_UART5_INDEX] = stm32_create_uart_dev(STM32_UART5,
  //    rcc_dev, gpio_dev, afio_dev, 0x40005000, pic[STM32_UART5_IRQ]);
  //    stm32_uart[STM32_UART6_INDEX] = stm32_create_uart_dev(STM32_UART6,
  //    rcc_dev, gpio_dev, afio_dev, 0x40011400, pic[STM32_UART6_IRQ]);
  ...
  struct {
    ...
    uint8_t irq_idx;
      ...
      {0x40013000, STM32_SPI1_IRQ}, {0x40003800, STM32_SPI2_IRQ},
      {0x40003CD0, STM32_SPI3_IRQ}, {0x40013400, STM32_SPI4_IRQ},
      {0x40015000, STM32_SPI5_IRQ}, {0x40015400, STM32_SPI6_IRQ},
  };
  ...
  stm32_init_periph(stm->qspi_dev, STM32_QSPI, 0xA0001000,
                    qdev_get_gpio_in(nvic, STM32_QSPI_IRQ));
...
#define dummy_dev(name, start, size)                                           \
  ...
  struct {
    ...
    uint8_t irq_idx;
      ...
      {1, 0x40010000,
       0}, // TIM1 FIXME: TIM1 is a complex timer w/ multiple IRQs
      ...
      {8, 0x40010400,
       0}, // TIM8 FIXME: TIM8 is a complex timer w/ multiple IRQs
