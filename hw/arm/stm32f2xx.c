...
static const char *stm32f2xx_periph_name_arr[] = {
    ENUM_STRING(STM32_UART1),       ENUM_STRING(STM32_UART2),
    ENUM_STRING(STM32_UART3),       ENUM_STRING(STM32_UART4),
    ENUM_STRING(STM32_UART5),       ENUM_STRING(STM32_UART6),

    ENUM_STRING(STM32_SPI1),        ENUM_STRING(STM32_SPI2),
    ENUM_STRING(STM32_SPI3),

    ENUM_STRING(STM32_I2C1),        ENUM_STRING(STM32_I2C2),
    ENUM_STRING(STM32_I2C3),

    ENUM_STRING(STM32_TIM1),        ENUM_STRING(STM32_TIM2),
    ENUM_STRING(STM32_TIM3),        ENUM_STRING(STM32_TIM4),
    ENUM_STRING(STM32_TIM5),        ENUM_STRING(STM32_TIM6),
    ENUM_STRING(STM32_TIM7),        ENUM_STRING(STM32_TIM8),
    ENUM_STRING(STM32_TIM9),        ENUM_STRING(STM32_TIM10),
    ENUM_STRING(STM32_TIM11),       ENUM_STRING(STM32_TIM12),
    ENUM_STRING(STM32_TIM13),       ENUM_STRING(STM32_TIM14),

    ENUM_STRING(STM32_GPIOA),       ENUM_STRING(STM32_GPIOB),
    ENUM_STRING(STM32_GPIOC),       ENUM_STRING(STM32_GPIOD),
    ENUM_STRING(STM32_GPIOE),       ENUM_STRING(STM32_GPIOF),
    ENUM_STRING(STM32_GPIOG),       ENUM_STRING(STM32_GPIOH),
    ENUM_STRING(STM32_GPIOI),       ENUM_STRING(STM32_GPIOJ),
...
void stm32f2xx_init(ram_addr_t flash_size, ram_addr_t ram_size,
                    const char *kernel_filename, Stm32Gpio **stm32_gpio,
                    Stm32Uart **stm32_uart, Stm32Timer **stm32_timer,
                    DeviceState **stm32_rtc, uint32_t osc_freq,
                    uint32_t osc32_freq, struct stm32f2xx *stm, ARMCPU **cpu) {
  ...
  nvic = armv7m_translated_init(
      stm32_container, address_space_mem, flash_size * 1024, ram_size * 1024,
      0, /* default number of irqs */
      kernel_filename, kernel_load_translate_fn, NULL, "cortex-m3", cpu);
  ...
  if (dinfo) {
    f2xx_flash_register(blk_by_legacy_dinfo(dinfo), 1 * 0x08000000,
                        flash_size * 1024);
  }

  // Create alias at 0x08000000 for internal flash, that is hard-coded at
  // 0x00000000 in armv7m.c:
  // TODO: Let BOOT0 and BOOT1 configuration pins determine what is mapped at
  // 0x00000000, see SYSCFG_MEMRMP.

  MemoryRegionSection mrs = memory_region_find(
      address_space_mem, STM32_FLASH_ADDR_START, WORD_ACCESS_SIZE);
  MemoryRegion *flash_alias = g_new(MemoryRegion, 1);
  memory_region_init_alias(flash_alias, NULL, "stm32f2xx.flash.alias", mrs.mr,
                           0, flash_size * 1024);
  ...
  DeviceState **gpio_dev =
      (DeviceState **)g_malloc0(sizeof(DeviceState *) * STM32F2XX_GPIO_COUNT);
  ...
  qemu_irq nvic_wake_irq =
      qdev_get_gpio_in_named(DEVICE((*cpu)->env.nvic), "wakeup_in", 0);
  f2xx_gpio_wake_set((stm32f2xx_gpio *)(stm32_gpio[STM32_GPIOA_INDEX]), 0,
                     nvic_wake_irq);
  ...
  struct {
    ...
    uint8_t irq_idx;
      ...
      {0x40011000, STM32_UART1_IRQ}, {0x40004400, STM32_UART2_IRQ},
      {0x40004800, STM32_UART3_IRQ}, {0x40004C00, STM32_UART4_IRQ},
      {0x40005000, STM32_UART5_IRQ}, {0x40011400, STM32_UART6_IRQ},
  };
  for (i = 0; i < ARRAY_LENGTH(uart_desc); ++i) {
    ...
    //        qdev_prop_set_ptr(uart_dev, "stm32_check_tx_pin_callback", (void
    //        *)stm32_afio_uart_check_tx_pin_callback);
  ...
  //    stm32_uart[STM32_UART1_INDEX] = stm32_create_uart_dev(STM32_UART1,
  //    rcc_dev, gpio_dev, afio_dev, 0x40011000, qdev_get_gpio_in(nvic,
  //    STM32_UART1_IRQ]); stm32_uart[STM32_UART2_INDEX] =
  //    stm32_create_uart_dev(STM32_UART2, rcc_dev, gpio_dev, afio_dev,
  //    0x40004400, qdev_get_gpio_in(nvic, STM32_UART2_IRQ]);
  //    stm32_uart[STM32_UART3_INDEX] = stm32_create_uart_dev(STM32_UART3,
  //    rcc_dev, gpio_dev, afio_dev, 0x40004800, qdev_get_gpio_in(nvic,
  //    STM32_UART3_IRQ]); stm32_uart[STM32_UART4_INDEX] =
  //    stm32_create_uart_dev(STM32_UART4, rcc_dev, gpio_dev, afio_dev,
  //    0x40004C00, qdev_get_gpio_in(nvic, STM32_UART4_IRQ]);
  //    stm32_uart[STM32_UART5_INDEX] = stm32_create_uart_dev(STM32_UART5,
  //    rcc_dev, gpio_dev, afio_dev, 0x40005000, qdev_get_gpio_in(nvic,
  //    STM32_UART5_IRQ]); stm32_uart[STM32_UART6_INDEX] =
  //    stm32_create_uart_dev(STM32_UART6, rcc_dev, gpio_dev, afio_dev,
  //    0x40011400, qdev_get_gpio_in(nvic, STM32_UART6_IRQ]);
...
#define dummy_dev(name, start, size)                                           \
  do {                                                                         \
  ...
  sysbus_connect_irq(SYS_BUS_DEVICE(dma1), 0,
                     qdev_get_gpio_in(nvic, STM32_DMA1_STREAM0_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma1), 1,
                     qdev_get_gpio_in(nvic, STM32_DMA1_STREAM1_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma1), 2,
                     qdev_get_gpio_in(nvic, STM32_DMA1_STREAM2_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma1), 3,
                     qdev_get_gpio_in(nvic, STM32_DMA1_STREAM3_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma1), 4,
                     qdev_get_gpio_in(nvic, STM32_DMA1_STREAM4_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma1), 5,
                     qdev_get_gpio_in(nvic, STM32_DMA1_STREAM5_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma1), 6,
                     qdev_get_gpio_in(nvic, STM32_DMA1_STREAM6_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma1), 7,
                     qdev_get_gpio_in(nvic, STM32_DMA1_STREAM7_IRQ));

  DeviceState *dma2 = qdev_create(NULL, "f2xx_dma");
  stm32_init_periph(dma2, STM32_DMA2, 0x40026400, NULL);
  sysbus_connect_irq(SYS_BUS_DEVICE(dma2), 0,
                     qdev_get_gpio_in(nvic, STM32_DMA2_STREAM0_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma2), 1,
                     qdev_get_gpio_in(nvic, STM32_DMA2_STREAM1_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma2), 2,
                     qdev_get_gpio_in(nvic, STM32_DMA2_STREAM2_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma2), 3,
                     qdev_get_gpio_in(nvic, STM32_DMA2_STREAM3_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma2), 4,
                     qdev_get_gpio_in(nvic, STM32_DMA2_STREAM4_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma2), 5,
                     qdev_get_gpio_in(nvic, STM32_DMA2_STREAM5_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma2), 6,
                     qdev_get_gpio_in(nvic, STM32_DMA2_STREAM6_IRQ));
  sysbus_connect_irq(SYS_BUS_DEVICE(dma2), 7,
                     qdev_get_gpio_in(nvic, STM32_DMA2_STREAM7_IRQ));
}
