...
void qmp_guest_set_time(bool has_time, int64_t time_ns, Error **errp) {
  ...
  if (pid == 0) {
    ...
    execle("/sbin/hwclock", "hwclock", has_time ? "-w" : "-s", NULL, environ);
    _exit(EXIT_FAILURE);
  } else if (pid < 0) {
    error_setg_errno(errp, errno, "failed to create child process");
    return;
...
static int find_open_flag(const char *mode_str, Error **errp) {
  unsigned mode;
...
#define DEFAULT_NEW_FILE_MODE                                                  \
  (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH)

static FILE *safe_open_or_create(const char *path, const char *mode,
                                 Error **errp) {
  Error *local_err = NULL;
  int oflag;
...
static void decode_mntname(char *name, int len) {
  ...
  for (i = 0; i <= len; i++) {
    if (name[i] != '\\') {
      ...
      name[j++] = '\\';
      i++;
    } else if (name[i + 1] >= '0' && name[i + 1] <= '3' && name[i + 2] >= '0' &&
               name[i + 2] <= '7' && name[i + 3] >= '0' && name[i + 3] <= '7') {
      name[j++] = (name[i + 1] - '0') * 64 + (name[i + 2] - '0') * 8 +
                  (name[i + 3] - '0');
      i += 3;
    } else {
      name[j++] = name[i];
...
static void build_fs_mount_list(FsMountList *mounts, Error **errp) {
  ...
  while (getline(&line, &n, fp) != -1) {
    ret = sscanf(line, "%*u %*u %u:%u %*s %n%*s%n%c", &devmajor, &devminor,
                 &dir_s, &dir_e, &check);
    if (ret < 3) {
      continue;
    }
    ...
    ret = sscanf(dash, " - %n%*s%n %n%*s%n%c", &type_s, &type_e, &dev_s, &dev_e,
                 &check);
    if (ret < 1) {
      continue;
    }
...
static void build_guest_fsinfo_for_real_device(char const *syspath,
  ...
  if (!p || sscanf(p + 12, "%*x:%*x/%x:%x:%x.%x%n", pci, pci + 1, pci + 2,
                   pci + 3, &pcilen) < 4) {
  ...
  if (p &&
      sscanf(p + 7, "%*u:%*u:%*u/%*u:%u:%u:%u", tgt, tgt + 1, tgt + 2) == 3) {
    has_tgt = true;
  }
...
static GuestFilesystemInfo *build_guest_fsinfo(struct FsMount *mount,
  ...
  char *devpath =
      g_strdup_printf("/sys/dev/block/%u:%u", mount->devmajor, mount->devminor);
...
GuestFilesystemTrimResponse *qmp_guest_fstrim(bool has_minimum, int64_t minimum,
  ...
  QTAILQ_FOREACH(mount, &mounts, next) {
    ...
    fd = qemu_open(mount->dirname, O_RDONLY);
    if (fd == -1) {
      result->error = g_strdup_printf("failed to open: %s", strerror(errno));
    ...
    if (ret == -1) {
      ...
      if (errno == ENOTTY || errno == EOPNOTSUPP) {
        ...
        result->error = g_strdup_printf("failed to trim: %s", strerror(errno));
      }
      close(fd);
      continue;
    }
...
static void bios_supports_mode(const char *pmutils_bin, const char *pmutils_arg,
  ...
  default:
    error_setg(errp,
               ...
               " code (%d)",
               pmutils_path, WEXITSTATUS(status));
    goto out;
  }
...
void qmp_guest_suspend_hybrid(Error **errp) {
  ...
  bios_supports_mode("pm-is-supported", "--suspend-hybrid", NULL, &local_err);
  if (local_err) {
    error_propagate(errp, local_err);
    return;
  }
...
guest_find_interface(GuestNetworkInterfaceList *head, const char *name) {
  ...
  for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
    ...
    if (!info->value->has_hardware_address && ifa->ifa_flags & SIOCGIFHWADDR) {
      ...
      if (ioctl(sock, SIOCGIFHWADDR, &ifr) == -1) {
        error_setg_errno(errp, errno, "failed to get MAC address of %s",
                         ifa->ifa_name);
        close(sock);
        goto error;
      }
      ...
      info->value->hardware_address =
          g_strdup_printf("%02x:%02x:%02x:%02x:%02x:%02x", (int)mac_addr[0],
    ...
    if (ifa->ifa_addr && ifa->ifa_addr->sa_family == AF_INET) {
      ...
      if (ifa->ifa_netmask) {
        ...
        address_item->value->prefix =
            ctpop32(((uint32_t *)p)[0]) + ctpop32(((uint32_t *)p)[1]) +
            ctpop32(((uint32_t *)p)[2]) + ctpop32(((uint32_t *)p)[3]);
      }
    }
  ...
  if (dirfd == -1) {
    ...
    if (fd == -1) {
      if (errno != ENOENT) {
        ...
        error_setg(errp,
                   "logical processor #%" PRId64 " can't be "
                   "offlined",
                   vcpu->logical_id);
...
static void transfer_memory_block(GuestMemoryBlock *mem_blk, bool sys2memblk,
  ...
  if (dirfd == -1) {
    if (sys2memblk) {
      ...
      if (errno == ENOENT) {
        ...
        result->response = GUEST_MEMORY_BLOCK_RESPONSE_TYPE_OPERATION_FAILED;
      }
    }
    g_free(dirpath);
    goto out1;
  ...
  if (sys2memblk) {
    ...
    if (mem_blk->online != (strncmp(status, "online", 6) == 0)) {
      char *new_state =
          mem_blk->online ? g_strdup("online") : g_strdup("offline");
...
GuestMemoryBlockList *qmp_guest_get_memory_blocks(Error **errp) {
  ...
  while ((de = readdir(dp)) != NULL) {
    ...
    if ((strncmp(de->d_name, "memory", 6) != 0) || !(de->d_type & DT_DIR)) {
      continue;
    }
...
void qmp_guest_suspend_ram(Error **errp) { error_setg(errp, QERR_UNSUPPORTED); }
...
#if !defined(__linux__)
  {
    const char *list[] = {"guest-suspend-disk",
                          "guest-suspend-ram",
...
#if !defined(CONFIG_FSFREEZE)
  {
    const char *list[] = {"guest-get-fsinfo",
